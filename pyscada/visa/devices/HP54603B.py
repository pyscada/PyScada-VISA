...
class Handler(GenericDevice):
    ...
    def mdo_set_horizontal_scale(self, time_per_div, **kwargs):
        self.inst.query(
            ":TIMEBASE:MODE NORM;:TIMebase:RANGe %s;*OPC?"
            % str(float(time_per_div) * 10.0)
        )
    ...
    def mdo_horizontal_scale_in_period(self, period=1.0, frequency=1000, **kwargs):
        ...
        self.mdo_set_horizontal_scale(
            mdo_horiz_scale, period=period, frequency=frequency
        )

    def mdo_set_vertical_scale(self, ch=1, value=1.0, **kwargs):
        self.inst.query(
            ":CHAN%d:RANGe %s;*OPC?" % (ch, str(value * mdo_horiz_div_quantity))
        )
        self.inst.query(":RUN;*OPC?")
        time.sleep(2 * kwargs.get("period", 1) / kwargs.get("frequency", 1000))
        self.inst.query(":STOP;*OPC?")

    def mdo_find_vertical_scale(
        self, ch=1, frequency=1000, range_i=None, period=4.0, **kwargs
    ):
        ...
        while range_i < len(vranges):
            ...
            self.mdo_set_vertical_scale(
                ch, vranges[range_i], period=period, frequency=frequency
            )
            data = self.mdo_query_waveform(
                ch=ch, frequency=frequency, refresh=True, period=period
            )
            if data is None:
                failed += 1
                if failed > 3:
            ...
            if range_i == 0:
                ...
                continue
            if (
                (mdo_horiz_div_quantity * 0.9 * vranges[range_i - 1] / 2.0)
                <= np.max(abs(data))
                < (mdo_horiz_div_quantity * 0.9 * vranges[range_i] / 2.0)
            ):
            ...
            if (
                np.where(
                    vranges > 2.0 * np.max(abs(data)) / mdo_horiz_div_quantity * 0.9
                )[0].size
                == 0
            ):
                continue
        ...
        self.inst.query(
            ":CHAN%d:RANGe %s;*OPC?" % (ch, str(np.max(abs(data)) * 2 / 0.7))
        )
        self.inst.query(":RUN;*OPC?")
    ...
    def mdo_gain(self, source1=1, source2=2, frequency=10, period=1.0, **kwargs):
        return 20 * np.log10(
            self.mdo_query_peak_to_peak(ch=source2, frequency=frequency, period=period)
            / self.mdo_query_peak_to_peak(
                ch=source1, frequency=frequency, period=period
            )
        )
    ...
    def mdo_prepare(self, **kwargs):
        ...
        self.inst.query(
            ":ACQuire:TYPE NORM;:TRIGger:COUPling DC;:TRIGger:MODE NORM;"
            ":CHAN1:OFFSet 0;:CHAN2:OFFSet 0;*OPC?"
        )
        self.inst.query(
            ":CHANnel1:COUPling DC;:CHANnel2:COUPling DC;:VIEW CHAN1;:VIEW CHAN2;*OPC?"
        )

    def mdo_query_waveform(
        self, ch=1, points_resolution=100, frequency=1000, period=4.0, **kwargs
    ):
        # io config
        points_resolution = (
            points_resolution
            if points_resolution in [100, 200, 250, 400, 500, 1000, 2000]
            else 2000
        )
        ...
        vpos = float(
            self.inst.query(":WAVeform:YREFerence?")
        )  # reference position (level)

        unscaled_wave = np.array(bin_wave, dtype="double")  # data type conversion
        scaled_wave = [
            (a - vpos) * vscale if a > 0 else (a + vpos) * vscale if a < 0 else a
            for a in unscaled_wave
        ]

        scaled_wave_mini = list()
        if points_resolution < int(len(scaled_wave)):
            for i in range(0, points_resolution):
                scaled_wave_mini.append(
                    scaled_wave[i * int(len(scaled_wave) / points_resolution)]
                )
        else:
    ...
    def mdo_get_phase(self, frequency=1000, period=4.0, **kwargs):
        ...
        for i in range(0, 10):
            time.sleep((i + 1) / 10)
            a = self.mdo_query_waveform(
                ch=1,
                frequency=frequency,
                refresh=True,
                points_resolution=10000,
                period=period,
            )
            b = self.mdo_query_waveform(
                ch=2,
                frequency=frequency,
                refresh=False,
                points_resolution=10000,
            ...
            phase1 = self.find_phase_2_signals(
                a, b, frequency, self.mdo_horizontal_time()
            )
            time.sleep((i + 1) / 10)
            a = self.mdo_query_waveform(
                ch=1,
            ...
            phase2 = self.find_phase_2_signals(
                a, b, frequency, self.mdo_horizontal_time()
            )
            time.sleep((i + 1) / 10)
            a = self.mdo_query_waveform(
            ...
            phase3 = self.find_phase_2_signals(
                a, b, frequency, self.mdo_horizontal_time()
            )
            ...
            if check_phase < 0.1 or mean_phase == 0:
                ...
                logger.debug(
                    "%s check_phase too big : %s (%s %s %s)"
                    % (i, check_phase, phase1, phase2, phase3)
                )
        self.inst.query(":RUN;*OPC?")
    ...
    def fft(self, eta):
        ...
        spectrum_hanning = (
            spectrum_hanning * 2 * 2 / nfft
        )  # also correct for Hann filter
    ...
    def find_phase_2_signals(self, a, b, frequency, tmax):
        ...
        recovered_phase_shift = (
            -1 * 2 * np.pi * (((0.5 + recovered_time_shift / period) % 1.0) - 0.5)
        )
