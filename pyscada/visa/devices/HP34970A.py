...
class HP34970A(object):
    ...
    def connect(self, instr):
        ...
        self.instr.write(
            "SYST:TIME %s" % (datetime.now().strftime("%H,%M,%S.000"))
        )  # set unit time
        self.instr.write(
            "SYST:DATE %s" % (datetime.now().strftime("%Y,%m,%d"))
        )  # set unit date
        self.instr.write("FORM:READ:TIME:TYPE ABS")
        dispstring = "Booting."
        for i in range(len(dispstring) - 13):
            self.instr.write(
                'DISP:TEXT "%s"' % dispstring[i : i + 13], encoding="ascii"
            )
            sleep(0.1)
    ...
    def _parse_result(self, result):
        for i in range(0, len(result), 8):
            ...
            self._channels[channel].timestamp = datetime(
                int(result[i + 1]),
                int(result[i + 2]),
                int(result[i + 3]),
                int(result[i + 4]),
...
class Handler(GenericDevice):
    ...
    def read_data_and_time(self, variable_instance):
        ...
        (
            data_type,
            channel,
        ) = variable_instance.visavariable.device_property.upper().split(";")
